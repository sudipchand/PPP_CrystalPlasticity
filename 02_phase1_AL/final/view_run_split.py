#!/usr/bin/env python3
"""
view_run_split.py - same as view_run.py but saves THREE separate images:

  <out>_newton_residual.png   - residual after the inner Newton loop (NWEND)
  <out>_fp_residual.png       - residual after the fixed-point loop (FPEND)
  <out>_active_systems.png    - active slip-system count at convergence (FPEND)
...
Usage:
  ...
  python view_run_split.py cp_uniaxial_1.feh --label phase1
  python view_run_split.py cp_uniaxial_1.feh --out myrun     # -> myrun_newton_residual.png etc.
  python view_run_split.py cp_uniaxial_1.feh --tail 300       # zoom last N records
"""
...
def main():
    ...
    ap.add_argument("--out", default="view_run",
                    help="output prefix; three files are written with suffixes")
    args = ap.parse_args()
    ...
    if nw.size:
        ...
        fig.tight_layout()
        out1 = f"{args.out}_newton_residual.png"
        fig.savefig(out1, dpi=150)
        plt.close(fig)
        print(f"saved: {out1}")

    # ---- Image 2: Fixed-point residual (FPEND) ----
    if fp.size:
        fig, ax = plt.subplots(figsize=(11, 5))
        ...
        ax.grid(True, which="both", alpha=0.3)
        fig.tight_layout()
        out2 = f"{args.out}_fp_residual.png"
        fig.savefig(out2, dpi=150)
        plt.close(fig)
        print(f"saved: {out2}")

    # ---- Image 3: Active slip-system count (FPEND) ----
    if fpact.size:
        fig, ax = plt.subplots(figsize=(11, 5))
        ax.plot(np.arange(fpact.size), fpact, "s-", ms=3, lw=0.8,
                color="tab:green")
        ...
        fig.tight_layout()
        out3 = f"{args.out}_active_systems.png"
