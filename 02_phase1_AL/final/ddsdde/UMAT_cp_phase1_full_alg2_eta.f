  ...
  DO WHILE (n_conv_fp)
     ...
     ELSE
        ...
        IF (kkt_ok) THEN
           ...
           WRITE(16,'(A,1X,I4,1X,ES16.8,1X,I3,1X,ES16.8)') 'CONVRG',l_iter_fp,&
                &SQRT(DOT_PRODUCT(residual_nw,residual_nw)),COUNT(active_slip_system),eta
           ! --- END DIAG ---
        ELSE
...
SUBROUTINE UMAT(STRESS,STATEV,DDSDDE,SSE,SPD,SCD,RPL,DDSDDT,DRPLDE,&
  ...
  CASE ('ABAQ')
     ...
     !
     ! --- DIAG: dump full DDSDDE (NTENS*NTENS) with increment number ---
     ! format: DDSDDE KINC NTENS  then all entries row-major
     WRITE(16,'(A,1X,I6,1X,I2,36(1X,ES16.8))') 'DDSDDE',KINC,NTENS,&
          &((DDSDDE(i_diag,j_diag),j_diag=1,NTENS),i_diag=1,NTENS)
     ! --- END DIAG ---
     !
  CASE('FEAP')
