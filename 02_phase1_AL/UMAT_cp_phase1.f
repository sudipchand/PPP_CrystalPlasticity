...
SUBROUTINE determine_incremental_lagrange_multiplier_augmented_lagrange(&
  ...
  REAL(KIND=rk)::norm_R,norm_Phi_tilde_inf,Phi_ref
  LOGICAL::kkt_ok
  ...
  DO WHILE (n_conv_fp)
     ...
     ! --- DIAG: residual after Newton loop ends (per fixed-point iter) ---
     ...
     ELSE
        ! successful back-tracking Newton iteration
        ...
        d_delta_lambda_ip1_d_C=comp_d_delta_lambda_ip1_d_C(&
             ...
             &env,NDI,NSHR)
        !
        ! update memory function for fix-point residual
        diff_delta_lambda=delta_lambda_ip1-delta_lambda_i
        norm_diff_delta_lambda_i_ip1=SQRT(DOT_PRODUCT(diff_delta_lambda,&
             &diff_delta_lambda))
        !
        ! update pseudo-viscosity for regularization and iteration counter
        ! for fixed-point iteration
        IF (l_iter_fp>n_memory) THEN
           aver_red_fp=SUM(mem_red_fp)/REAL(n_memory)
           mem_mult_fact(TWO_int:n_memory)=mem_mult_fact(ONE_int:(n_memory-ONE_int))
           IF (norm_diff_delta_lambda_i_ip1>aver_red_fp) THEN
              mem_mult_fact(ONE_int)=TWO_by_THREE_r*mult_fact_init
           ELSE
              mem_mult_fact(ONE_int)=mult_fact_init
           END IF
           mult_fact=SUM(mem_mult_fact)/REAL(n_memory)
        END IF
        eta_save=eta
        eta=eta*mult_fact
        !
        mem_red_fp(TWO_int:n_memory)=mem_red_fp(ONE_int:(n_memory-ONE_int))
        mem_red_fp(ONE_int)=norm_diff_delta_lambda_i_ip1
        l_iter_fp=l_iter_fp+ONE_int
        !
        ! convergence check or update
        ! Phi_tilde_plus, Eq.(37); guard dt*eta against zero
        Phi_tilde_plus=MAX(Phi_alpha_all,-delta_lambda_ip1/MAX(dt*eta,lb_invert))
        norm_Phi_tilde_inf=MAXVAL(ABS(Phi_tilde_plus))
           
        !
        ! reference stress to non-dimensionalize Phi (units: MPa here).
        ! mat_param_inelast(1) is the initial slip resistance tau0.
        Phi_ref=MAX(ABS(mat_param_inelast(ONE_int)),lb_invert)
        ! KKT check, Alg.2 line 8 (Phi-conditions scaled by Phi_ref):
        ! Phi/Phi_ref <= eps_c, dl >= -eps_c, |Phi*dl|/Phi_ref <= eps_c
        ...
        !
        IF (kkt_ok.AND.(norm_diff_delta_lambda_i_ip1<tol_fp)) THEN
           n_conv_fp=.FALSE.
           ! --- DIAG: residual after fixed-point loop ends (per increment) ---
           WRITE(16,'(A,1X,I4,1X,ES16.8,1X,I3)') 'FPEND',l_iter_fp,&
                &SQRT(DOT_PRODUCT(residual_nw,residual_nw)),COUNT(active_slip_system)
           ! --- END DIAG ---
        ELSE
           ! adaptive Newton tolerance update, Eq.(39)
           tol_nw=MAX(eps_min_nw,MIN(ONE_HUNDREDTH_r*tol_nw,norm_Phi_tilde_inf/Phi_ref))
...
SUBROUTINE determine_incremental_lagrange_multiplier_miehe_viscoplastic(&
  ...
  REAL(KIND=rk),DIMENSION(24,6)::d_delta_lambda_i_d_C
  REAL(KIND=rk),DIMENSION(24,24)::jacobian_nw
  REAL(KIND=rk),DIMENSION(24)::Phi_alpha_all
  INTEGER(KIND=ik),INTENT(IN)::algo,symm_class,NDI,NSHR,hard_law
  ...
  IF (.NOT.(conv_issue)) THEN
     ...
     CALL residual_tangent_computation_nw_crystal_plasticity(dummy_var,&
          ...
          &delta_lambda_i,delta_lambda_ip1,algo,dt,m_SH,elast_const,&
          &symm_class,orth_basis,orth_proj,mat_param_inelast,NDI,NSHR,&
          &env,eta,hard_law,theta_KK,Phi_alpha_all)
  ELSE
     delta_lambda_ip1=ZERO_r
